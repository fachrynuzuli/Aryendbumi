...
This prompt evaluates a business idea by combining market research insights with an execution feasibility assessment to determine if the concept is both desirable to the target audience and practical to bring to market. To use it, replace {BUSINESS\_IDEA} with a clear description of your idea and {MARKET\_RESEARCH\_REPORT} with your market findings (such as outputs from the Prompt 4: Deep Market Research for \<INDUSTRY\>). The analysis will identify fit with market needs, highlight opportunities and risks, outline revenue paths, and provide an actionable short-term roadmap.
